function segList = ExtractSceneSegments(polarLidarPoints, toleranceInsideSegment, nbSegmentPtMinimum)
    ...
    for i = 1:size(VertexList, 1)
        while ~isequal(VertexList(i,:), polarLidarPoints(j,:))
            j = j + 1;
            if j > size(polarLidarPoints, 1)
                j = 1;
            end
        end
        endOfSegmentIndex(end + 1) = j;
    end
    ...
    for i = 1:size(VertexList, 1)
        ...
        ptDebut = [vertexDebutSegment(1), vertexDebutSegment(2)];
        ptFin = [vertexFinSegment(1), vertexFinSegment(2)];

        
        angleSegment = Angle([0, 0; ptFin], [ptDebut; ptFin]);
    ...
    for i = 1:length(segmentList)
        ...
        if abs(moduloPi(s.angleSegmentRefLidar)) >= deg2rad(20)
            if s.indexPtLidarFinSegment - s.indexPtLidarDebutSegment >= nbSegmentPtMinimum
                if i ~= 1
                    s.angleSegmentRefLidarPrecedent = segmentList{i - 1}.angleSegmentRefLidar;
                else
                    s.angleSegmentRefLidarPrecedent = segmentList{end}.angleSegmentRefLidar;
                end

                if i < length(segmentList)
                    s.angleSegmentRefLidarSuivant = segmentList{i + 1}.angleSegmentRefLidar;
                else
                    s.angleSegmentRefLidarSuivant = segmentList{1}.angleSegmentRefLidar;
                end
                segList{end + 1} = s;
            end
        end
    ...
    % Find the segment indices based on vertex points
    for i = 1:size(vertexList, 1)
    ...
    % Filter segments based on angle and other conditions
    filteredSegList = {};
    for i = 1:length(segList)
        s = segList{i};
        if abs(ModuloPi(s.angleSegmentRefLidar)) >= deg2rad(20)
            
                if s.indexPtLidarFinSegment - s.indexPtLidarDebutSegment >= 5
